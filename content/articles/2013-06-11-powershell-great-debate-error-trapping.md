...
There are a few different approaches folks take to trapping an error (I'm not discussingÂ _capturing_ the error, just knowing that one occurred).  
...
`A "con" of this approach is that $? doesn't indicate an error. It indicates whether or notÂ _the previous commandÂ 
thinks
 it completed successfully.Â _It's reliable withÂ _most_ cmdlets - but I've seen it fail for a lot of external utilities. Given that it isn't 100% reliable as an indicator, I tend to shy away from it. I'd rather learn one way that always works, and that's been Try/Catch for me.  
Try/Catch also makes it easy to catch different exceptions differently. I don't always need to do so... but again, I'd rather learnÂ _one_ way to do things thatÂ _always_ works and provides more flexibility. I don't want to use $? sometimes, and then use something else other times, because that's more to remember, teach, learn, etc.  
Some folks will do anÂ **$error.clear()**, clearing the error collection, and then run a command. They'll then checkÂ **$error.count** to see if it's nonzero. I don't like that as much because it looks messy to me, and again - it doesn't let me easily handle different exceptions as easily as Try/Catch.  
Ok... your thoughts?  
