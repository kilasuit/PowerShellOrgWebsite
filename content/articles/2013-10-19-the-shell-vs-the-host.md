...
One thing that's oftenÂ _very_ confusing about PowerShell is the difference between the shell itself - what I'll callÂ _the engine_ in this article - and the application that hosts the engine.  
You see, you as a human being can't really interact directly with PowerShell's engine. Instead, you need aÂ _host application_ that lets you do so. The standard console - PowerShell.exe - is one such host; the Integrated Script Environment (ISE) is another. Those hosts "spin up" aÂ _runspace,Â _which is essentially an instance of the PowerShell engine. When you type a command and hit enter, the host creates a pipeline, jams your command into it, and then deals with the output.  
