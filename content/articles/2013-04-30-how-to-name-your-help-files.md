...
For scripts and functions, you can write [comment-based help][1] (aka "inline help"). Â All parameters of the Get-Help cmdlet support comment-based help, including the new ShowWindow parameter, and by adding a URL to the first related link, you can support the Online parameter in comment-based help.  
...
Get-Help looks for the XML-based help topics for the commands in a module in language-specific subdirectories of the module's installation directory.Â  This is generally well-known and an easy instruction to follow.  
...
For example:
  ...
  * System.Management.Automation.dll-help.xmlÂ Â Â  #Cmdlets, providers
  * MSFT_NetIPAddress.cdxml-help.xmlÂ Â Â Â Â Â Â Â Â Â Â Â Â # CIM commands
  * RemoteDesktop.psm1-help.xmlÂ Â Â Â Â Â Â Â Â Â Â Â Â Â Â Â Â Â # Functions, Script workflows

Here are the specifics:

  * **Cmdlets**:Â  Help topics for cmdlets must be in a file that is named for the _assembly_ in which the cmdlet is defined.
  * **Providers**:Â  Just like cmdlets, the help topics for providers must be in a file that is named for the assembly in which the provider is defined. The order in which cmdlet and provider help topics appear in the XML file doesn't matter a bit.
...
Easy, right? Now it gets a bit weird.
  ...
  * **Functions**:Â  Get-Help looks in the function code for an **.ExternalHelp** comment. The value of the comment is the help file name. If there's no ExternalHelp comment, Get-Help cannot find the XML based help file, no matter where it is or what it's named.Get-Help does not require a particular name for function help files, but they're typically named for the script module in which they are defined, such as MyModule.psm1-help.xml.For example: 
