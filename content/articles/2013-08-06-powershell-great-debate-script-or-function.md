...
Let's be clear: if an assignment explicitly asks for a function, you should write one. What we're debating are the pros and cons of aÂ _single tool_ being written one way or another. Read that again:Â _a single tool.Â _If you're writing a library of tools, it's obvious that writing them as functions for inclusion in a single file (like a script module) is beneficial.  
